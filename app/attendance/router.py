...
from app.database import get_db  # Connect to Member 3's database dependency.
...
async def get_current_principal() -> UserPrincipal:
    """Replace with Member 3's JWT authentication dependency."""
    raise NotImplementedError("Connect this to the JWT dependency")

def require_role(*roles: str):
    async def guard(user: UserPrincipal = Depends(get_current_principal)):
...
def digest(token: str) -> str: return sha256(token.encode()).hexdigest()
...
def valid_token_session(db: Session, raw: str) -> tuple[AttendanceSession, QRToken]:
    item = db.scalar(select(QRToken).where(QRToken.token_hash == digest(raw)))
...
def start_session(payload: StartSession, db: Session = Depends(get_db), user: UserPrincipal = Depends(require_role("faculty"))):
...
def issue_qr(session_id: int, db: Session = Depends(get_db), user: UserPrincipal = Depends(require_role("faculty"))):
    ...
    db.add(QRToken(session_id=session.id, token_hash=digest(raw), expires_at=expires_at)); db.commit()
    return {"token": raw, "expires_at": expires_at}

@router.post("/qr/validate")
def validate_qr(payload: TokenRequest, db: Session = Depends(get_db), user: UserPrincipal = Depends(require_role("student"))):
...
def mark_attendance(payload: TokenRequest, db: Session = Depends(get_db), user: UserPrincipal = Depends(require_role("student"))):
...
def end_session(session_id: int, db: Session = Depends(get_db), user: UserPrincipal = Depends(require_role("faculty"))):
    session = active_session(db, session_id)
    if integrations.faculty_id_for_user(db, user.user_id) != session.faculty_id: raise HTTPException(403, "This is not your session")
...
def live_attendance(session_id: int, db: Session = Depends(get_db), user: UserPrincipal = Depends(require_role("faculty"))):
...
def my_history(db: Session = Depends(get_db), user: UserPrincipal = Depends(require_role("student"))):
...
def course_history(course_id: int, db: Session = Depends(get_db), user: UserPrincipal = Depends(require_role("faculty"))):
    faculty_id = integrations.faculty_id_for_user(db, user.user_id)
